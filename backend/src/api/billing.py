...
async def checkout(body: CheckoutRequest, user_id: UUID = Depends(get_current_user_id), db: AsyncSession = Depends(get_db)) -> dict:
...
async def webhook(request: Request, db: AsyncSession = Depends(get_db)) -> dict:
    ...
    event = stripe.Webhook.construct_event(payload, sig, settings_billing.stripe_webhook_secret)
...
async def subscription(user_id: UUID = Depends(get_current_user_id), db: AsyncSession = Depends(get_db)) -> SubscriptionResponse:
    result = await db.execute(select(Subscription).where(Subscription.user_id == user_id))
